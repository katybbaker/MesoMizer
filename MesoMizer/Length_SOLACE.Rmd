...
ggplot(Lengths, aes(x = Length_mm, fill = Taxa_updated)) +
...
bin_lengths <- function(df, n_bins = 10) {
  breaks <- quantile(df$Length_mm, probs = seq(0, 1, length.out = n_bins + 1), na.rm = TRUE)
  # Remove duplicate breaks
  breaks <- unique(breaks)
  
  # Only bin if we have at least 2 unique breakpoints
  if (length(breaks) > 1) {
    df$bin <- cut(df$Length_mm, breaks = breaks, include.lowest = TRUE)
  } else {
...
Abundance_binned <- Lengths_binned %>%
  left_join(Abundance, by = "Taxa_updated") %>%
  mutate(
    abundance_per_bin = proportion * abd_total_gm2_sp
  )

# View result
print(Abundance_binned)


ggplot(Abundance_binned, aes(x = bin, y = abundance_per_bin, fill = Taxa_updated)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~ Taxa_updated, scales = "free") +
  labs(
    x = "Size Bin",
    y = "Abundance (g m⁻²)",
    title = "Abundance per Size Bin by Taxa"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))


Abundance_binned <- Abundance_binned %>%
  mutate(
    Taxa_updated = case_when(
...
write.csv(Abundance_binned, "Abundance_persize.csv")
