...
ext_mort_rates <- getExtMort(params_v1) # use this -> multiply by 0.17 (ext_prop set by mizer shelf) -> multiply by each size class
...
ext_mort_rates_long <- ext_mort_rates_df %>%
  pivot_longer(cols = -Species, names_to = "Size", values_to = "ext_mort_rate") %>%
  mutate(Size = as.numeric(Size))


# 
# # Convert metabolic rates array to long-format data frame
# ext_mort_rates_df <- melt(ext_mort_rates, varnames = c("Species", "Size"), value.name = "ext_mort_Rate")

carcass_carbon_df <- ext_mort_rates_long %>% 
  mutate(g_C = Size * 0.4) %>% # change this to species specific carbon content
  mutate(carcass_C = g_C * ext_mort_rate * 0.1713287) #constant taken from MizerShelf - proportion of ext_mort that produces carrion
...
combined_df1 <- merge(respired_carbon_long, feces_df, by = c("Species", "Size"))
...
carbon_pools <- final_df %>% 
  ...
  mutate(total_POC = sum(Fecal_Carbon) +sum(carcass_C)) %>% 
