...
params_v1 <- readRDS("tuned_params_sp2.rds")
...
#                    "POC_Umeso" = "Umeso",
#                    "POC_Lmeso" = "Lmeso"),
#     DIC_retained = POC * Kmicrob,
#     POC_sinking = POC * (1-Kmicrob)
#   ) 
# 
# DIC_pools_df <- POC_DIC %>% 
#   select(Species, Layer, DIC_retained, POC_sinking) %>% 
#   group_by(Species) %>%
#   arrange(Species, Layer) %>%
#   mutate(
#     total_DIC = case_when(
#       Layer == "Epi" ~ DIC_retained,
#       Layer == "Umeso" ~ DIC_retained + lag(POC_sinking, default = 0),
#       Layer == "Lmeso" ~ DIC_retained + lag(POC_sinking, default = 0))) %>% 
#   ungroup()
# 
# 
# # Create the horizontal bar plot
# DIC_resp_pools <- ggplot(DIC_pools_df, aes(x = total_DIC, y = Layer, fill = Species)) +
#   geom_bar(stat = "identity") +
#   theme_minimal() +
#   labs(x = "DIC (g C m-2 d-1)", y = "Depth Stratum") +
#   scale_y_discrete(labels = c("Lmeso","Umeso","Epi")) +
