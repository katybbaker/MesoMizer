...
combined_plot <- ggplot(model_combined, aes(x = mod_type, y = mgC_m.2_d, fill = Species)) +
  ...
  scale_fill_manual(
    values = taxa_colors,
    labels = function(x) gsub("Crustacean \\(other\\)", "Crustaceans (other)", x)  # Change label
  ) +
  labs(x = "Model Type", y = "mg C m⁻² d⁻¹", fill = "Group") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text.x = element_text(angle = 0, size = 14),
    axis.text.y = element_text(size = 14),
    axis.title = element_text(size = 16),
    axis.title.x = element_text(margin = margin(t = 10)),  # Adds top margin to separate from axis text
...
```{r}
total_export <- model_combined %>% 
  group_by(mod_type) %>% 
  mutate(total_mg_C_m2_d = sum(mgC_m.2_d))
